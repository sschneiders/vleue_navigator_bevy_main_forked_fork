use bevy::{color::palettes, prelude::*, ecs::entity::EntityHashMap};
use rand::Rng;
...
pub fn give_target_to_navigator<const SIZE: u32, const X: u32, const Y: u32>(
    ...
    navmesh: Query<&NavMeshWrapper>,
    mut meshes: ResMut<Assets<Mesh>>,
    mut materials: ResMut<Assets<StandardMaterial>>,
) {
    for (entity, transform, navigator) in &navigators {
        let Some(navmesh) = navmeshes.get_mut(&navmesh.single().0) else {
            continue;
        };
...
pub fn move_navigator<const SIZE: u32>(
    ...
    for (mut transform, mut path, entity, mut navigator) in navigator.iter_mut() {
        ...
        transform.translation +=
            move_direction.normalize() * time.delta_secs() * navigator.speed;
